...
if (typeof marked !== 'undefined') {
    ...
    renderer.code = (code, lang) => {
        ...
        return `<div class="pre-wrap"><button class="copy-btn"><i class="fa-regular fa-copy"></i> Copy</button><pre class="language-${vl}"><code class="language-${vl}">${code}</code></pre></div>`;
    };
    renderer.heading = (text, level) => {
        const id = text.toLowerCase().replace(/[^\w]+/g,'-');
        return `<h${level} id="${id}">${text}<a class="heading-anchor" href="#${id}" title="Link to section">#</a></h${level}>`;
    };
    renderer.blockquote = (q) => {
        if (q.includes('[!NOTE]')||q.includes('[!INFO]')) return `<div class="alert-box info"><div><i class="fas fa-info-circle"></i></div><div>${q.replace(/\[!(NOTE|INFO)\]/g,'<strong>NOTE:</strong>')}</div></div>`;
        if (q.includes('[!WARNING]')) return `<div class="alert-box warning"><div><i class="fas fa-exclamation-triangle"></i></div><div>${q.replace(/\[!WARNING\]/g,'<strong>WARNING:</strong>')}</div></div>`;
        if (q.includes('[!IMPORTANT]')) return `<div class="alert-box"><div><i class="fas fa-exclamation-circle"></i></div><div>${q.replace(/\[!IMPORTANT\]/g,'<strong>IMPORTANT:</strong>')}</div></div>`;
        if (q.includes('[!CAUTION]')) return `<div class="alert-box danger"><div><i class="fas fa-shield-alt"></i></div><div>${q.replace(/\[!CAUTION\]/g,'<strong>CAUTION:</strong>')}</div></div>`;
...
function updateMeta(text) {
    ...
    if (el('meta-words')) el('meta-words').innerHTML = `<i class="fa-solid fa-align-left"></i> ${words.toLocaleString()} words`;
    if (el('meta-sections')) el('meta-sections').innerHTML = `<i class="fa-solid fa-layer-group"></i> ${sections} sections`;
    if (el('meta-read')) el('meta-read').innerHTML = `<i class="fa-regular fa-clock"></i> ${mins} min read`;
    if (el('topbar-reading-time')) { el('topbar-reading-time').style.display = 'flex'; el('reading-time-text').textContent = `${mins} min read`; }
}
...
function createToolbar(openWin, openFull, zoomEl) {
    ...
    let scale=1;
    const btns = [
        {icon:'fa-solid fa-arrow-up-right-from-square',label:'Open in Window',cls:'mtb-window',fn:openWin},
        {icon:'fa-solid fa-expand',label:'Fullscreen',cls:'mtb-fullscreen',fn:openFull},
        {icon:'fa-solid fa-magnifying-glass-plus',label:'Zoom In',cls:'mtb-zoomin',fn:()=>{ scale=Math.min(scale+.25,4); zoomEl.style.transform=`scale(${scale})`; zoomEl.style.transformOrigin='top center'; }},
        {icon:'fa-solid fa-magnifying-glass-minus',label:'Zoom Out',cls:'mtb-zoomout',fn:()=>{ scale=Math.max(scale-.25,.25); zoomEl.style.transform=`scale(${scale})`; zoomEl.style.transformOrigin='top center'; }},
    ];
...
async function loadDocument(docPath, title) {
    if(!docViewer) return;
    docViewer.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:400px;color:var(--text-muted);"><div style="font-size:2rem;margin-bottom:1rem;"><i class="fas fa-circle-notch fa-spin"></i></div><div>Loading <code>${docPath}</code>…</div></div>`;
    ...
    try {
        ...
        docViewer.innerHTML=`<div class="md-view">${marked.parse(currentDocText)}</div>`;
        ...
        docViewer.innerHTML=`<div class="alert-box danger"><div style="font-size:1.5rem;"><i class="fas fa-exclamation-circle"></i></div><div><h4 style="color:#fb7185;margin-bottom:8px;">Load Error</h4><p>Could not load: <code>${docPath}</code></p><pre style="margin-top:12px;background:rgba(0,0,0,.3)!important;"><code>${err.message}</code></pre></div></div>`;
    }
}
