...
function createToolbar(openWin, openFull, zoomEl) {
    ...
    let scale=1;
    const btns = [
        {icon:'fa-solid fa-arrow-up-right-from-square',label:'Window',cls:'mtb-window',fn:openWin},
...
async function loadDoc(link) {
    ...
    content.innerHTML = `<div class="mob-loading"><i class="fas fa-circle-notch fa-spin"></i><span>Loading…</span></div>`;
    try {
        const r = await fetch(docPath);
        if(!r.ok) throw new Error(`HTTP ${r.status}`);
        currentDocText = await r.text();
        const words = currentDocText.trim().split(/\s+/).length;
        const mins = Math.max(1,Math.round(words/200));
        content.innerHTML = `
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
                <span style="font-size:.72rem;color:var(--dim);display:flex;align-items:center;gap:4px;"><i class="fa-regular fa-clock"></i> ${mins} min read</span>
                <span style="font-size:.72rem;color:var(--dim);display:flex;align-items:center;gap:4px;"><i class="fa-solid fa-align-left"></i> ${words.toLocaleString()} words</span>
            </div>
            <div class="mob-md">${marked.parse(currentDocText)}</div>
            <div class="mob-doc-nav">
                <button class="mob-doc-btn" id="mob-doc-prev" style="display:none"><i class="fa-solid fa-arrow-left"></i><div><div class="dfn-label">Previous</div><div class="dfn-title">—</div></div></button>
                <button class="mob-doc-btn mob-doc-btn-next" id="mob-doc-next" style="display:none"><div><div class="dfn-label">Next</div><div class="dfn-title">—</div></div><i class="fa-solid fa-arrow-right"></i></button>
            </div>`;
