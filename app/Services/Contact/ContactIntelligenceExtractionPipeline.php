...
class ContactIntelligenceExtractionPipeline
{
    ...
    public function process(ContactAnalysisRun $run): void
    {
        try {
            ...
            $requestedKeys = [];
            if ($options['extract_topics'] ?? true) {
                $requestedKeys[] = 'topics (array of strings)';
            }
            if ($options['infer_persona'] ?? true) {
                $requestedKeys[] = 'persona (string summary)';
            }
            if ($options['detect_emotion'] ?? true) {
                $requestedKeys[] = 'emotional_baseline (string)';
            }
            if ($options['suggest_rules'] ?? true) {
                $requestedKeys[] = 'suggested_rules (array of strings)';
            }
            ...
            try {
                $agent = Agent::where('type', 'profiler')->orWhere('name', 'like', '%profiler%')->first() ?? new Agent;
