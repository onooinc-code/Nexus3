...
class NexusDashboardService
{
            ...
            return DB::table('contacts')
                ...
                ->map(function ($c) {
                    ...
                    return [
                        ...
                        'last_message_snippet' => $lastMsg ? Str::limit($lastMsg->content, 60) : 'No interactions recorded.',
    ...
    private function getAiUsage(User $user): array
    {
        try {
            ...
            $todayTokens = $this->safeCount(fn () => DB::table('usage_logs')
                ->where('timestamp', '>=', $today)
                ->sum(DB::raw('input_tokens + output_tokens')) ?? 0, 0);

            $monthTokens = $this->safeCount(fn () => DB::table('usage_logs')
                ->where('timestamp', '>=', $monthStart)
                ->sum(DB::raw('input_tokens + output_tokens')) ?? 0, 0);

            $todayCost = $this->safeCount(fn () => DB::table('usage_logs')
                ->where('timestamp', '>=', $today)
                ->sum('total_cost') ?? 0.0, 0.0);

            $monthCost = $this->safeCount(fn () => DB::table('usage_logs')
                ->where('timestamp', '>=', $monthStart)
                ->sum('total_cost') ?? 0.0, 0.0);
    ...
    public function getHealthStatus(): array
    {
        ...
        $services[] = $this->probeService('AI Models', function () {
            $geminiKey = env('GEMINI_API_KEY') ?: app(SettingCacheService::class)->get('gemini_api_key');
            $openaiKey = env('OPENAI_API_KEY') ?: app(SettingCacheService::class)->get('openai_api_key');
            $anthropicKey = env('ANTHROPIC_API_KEY') ?: app(SettingCacheService::class)->get('anthropic_api_key');

            $ok = false;
            $start = microtime(true);
            if ($geminiKey) {
                $response = Http::timeout(2)->get("https://generativelanguage.googleapis.com/v1beta/models?key={$geminiKey}");
                $ok = $response->successful();
            } elseif ($openaiKey) {
                $response = Http::timeout(2)->withHeaders(['Authorization' => "Bearer {$openaiKey}"])->get('https://api.openai.com/v1/models');
                $ok = $response->successful();
            } elseif ($anthropicKey) {
                $response = Http::timeout(2)->withHeaders(['x-api-key' => $anthropicKey])->get('https://api.anthropic.com/v1/models');
                $ok = $response->successful();
            }
