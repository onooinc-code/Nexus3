...
class LiveMsgsSyncService
{
    ...
    {
        $url = app(SettingCacheService::class)->get('waha_url')
    ...
    {
        return app(SettingCacheService::class)->get('waha_api_key')
    ...
    public function syncContacts($processId = null): void
    {
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                $existingIds = Contact::whereNotNull('waha_contact_id')->pluck('waha_contact_id')->toArray();
    ...
    public function syncMessages($processId = null): void
    {
        ...
        try {
            $contacts = Contact::whereNotNull('waha_contact_id')->get();
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($process) {
                    ...
                    broadcast(new JobProgressUpdated($process->id, 'sync_messages', 0, 0, $totalMessages, 'running', "Fetched {$totalMessages} messages, queuing chunks..."));
                }

                if ($totalMessages > 0) {
                    $chunks = array_chunk($messages, 500);
                    foreach ($chunks as $chunk) {
                        ProcessWahaMessageChunkJob::dispatch($contact->id, $chunk, $processId);
                    }
                    Log::info("Synced single contact {$chatId}: dispatched ".count($chunks)." chunks for {$totalMessages} messages.");
                } else {
                    if ($process) {
                        $process->update(['status' => 'completed', 'completed_at' => now(), 'progress' => 100]);
                        broadcast(new JobProgressUpdated($process->id, 'sync_messages', 100, 0, 0, 'completed', 'No messages found.'));
                    }
                }
